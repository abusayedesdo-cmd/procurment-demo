...
@section('content')
    <div class="shell">
        ...
        @isset($missingPlanForPr)
            <div class="missing-plan-notice">
                <div>
                    No Procurement Plan has been created yet for <b>PR-{{ $missingPlanForPr->pr_number ?? $missingPlanForPr->id }}</b> — a Procurement Plan is required before transferring it to a Sub-Committee.
                </div>
        ...
        @isset($missingSubCommitteeForProject)
        @if (in_array(auth()->user()->roleName(), [\App\Models\User::ADMIN, \App\Models\User::PROCUREMENT_OFFICER]))
            <div class="missing-subcommittee-panel">
                ...
                <div class="ssc-row">
                    <div>
                        ...
                        <input type="text" id="ssc_name" value="{{ $missingSubCommitteeForProject['suggested_name'] }}">
                    </div>
                    <div>
    ...
    @endisset
        ...
        @elseif (isset($cases))
            ...
            @if ($cases->isEmpty())
                <div class="group-panel">
                   <div class="no-cases">No procurement  yet. <a href="{{ route('cases.create', $activePr ? ['pr_id' => $activePr->id] : []) }}">Open a new case</a> to get started.</div>
                </div>
            @else
                <div class="case-grid">
